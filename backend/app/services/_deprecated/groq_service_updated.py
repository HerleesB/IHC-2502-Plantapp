"""Servicio para interactuar con Groq AI - VERSIÓN ACTUALIZADA CON MEJORA #1"""
import base64
import logging
...
# ========== MEJORA #1: Validación Rápida para Streaming ==========

async def validate_photo_quality_fast(image_bytes: bytes) -> Dict[str, Any]:
    """
    MEJORA #1: Validación RÁPIDA para streaming (optimizada para < 2 segundos).
    Usa prompt más corto y menos tokens que validate_photo_quality().
    ...
    # Prompt ULTRA-CORTO para baja latencia
    prompt = """Analiza RÁPIDAMENTE esta foto de planta.
...
Responde SOLO en JSON (sin markdown):
{
    ...
    "recommendations": {
        ...
        "lighting": "more/less/ok",
        "focus": "refocus/ok"
    },
    "voice_guidance": "Mensaje corto en español (máximo 10 palabras)"
}

Ejemplos de voice_guidance:
- Si planta arriba: "Mueve la cámara hacia arriba"
- Si planta abajo: "Baja la cámara un poco"
- Si muy lejos: "Acércate más"
- Si muy cerca: "Aléjate un poco"
- Si poca luz: "Busca más luz"
- Si todo ok: "Perfecto, lista para capturar"
"""
    
    # Analizar con parámetros optimizados para velocidad
    result = await service.analyze_image_with_prompt(
        image_bytes=image_bytes,
        prompt=prompt,
        temperature=0.1,  # Muy baja para respuestas rápidas
        max_tokens=150    # Mucho menos que validate_photo_quality (512)
    )
    
    if not result.get("success"):
        logger.error(f"Error en validación rápida: {result.get('error')}")
        return {
            "success": False,
            "guidance": "Error al analizar. Intenta de nuevo.",
            "details": {
                "lighting": 0.0,
                "focus": 0.0,
                "distance": 0.0,
                ...
                "recommendations": {"direction": "center", "distance": "ok", "lighting": "ok", "focus": "ok"}
            }
        }
    ...
    try:
        ...
        is_centered = validation_json.get("is_centered", False)
        plant_detected = validation_json.get("plant_detected", False)
        voice_guidance = validation_json.get("voice_guidance", "Ajusta la posición")
        recommendations = validation_json.get("recommendations", {})
        
        lighting_ok = recommendations.get("lighting") == "ok"
        focus_ok = recommendations.get("focus") == "ok"
        distance_ok = recommendations.get("distance") == "ok"
        
        lighting_score = 0.9 if lighting_ok else 0.4
        focus_score = 0.9 if focus_ok else 0.5
        distance_score = 0.9 if distance_ok else 0.5
        
        if is_centered and plant_detected and all([lighting_ok, focus_ok, distance_ok]):
            overall_score = 0.9
        elif is_centered and plant_detected:
            overall_score = 0.7
        elif plant_detected:
            overall_score = 0.5
        else:
        ...
        logger.info(f"Validación rápida: {'✅' if success else '❌'} (score: {overall_score:.2f})")
        ...
        logger.warning(f"Respuesta rápida no es JSON válido: {e}")
        return {
            "success": False,
            "guidance": "Centra la planta en la imagen",
            "details": {
                "lighting": 0.5,
                "focus": 0.5,
                "distance": 0.5,
        ...
        logger.error(f"Error procesando validación rápida: {e}")
        return {
            "success": False,
            "guidance": "Error al validar. Intenta de nuevo.",
            "details": {
                "lighting": 0.0,
                "focus": 0.0,
                "distance": 0.0,
